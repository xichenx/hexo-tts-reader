...
class ReaderPipeline {
  ...
  async processPost(post) {
    ...
    // When auto voice routing is on, the cache key must reflect the whole voice
    // map (and the fallback) so changing any mapping re-synthesizes. When it is
    // off, keep the plain voice id so existing caches stay valid.
    const voiceKey = this.config.autoVoice
      ? JSON.stringify({ auto: 1, voices: this.config.voices, fallback: this.config.voice })
      : this.config.voice;
    ...
    if (!ready) {
      try {
        const buf = await synthesize(usable, {
          ...
          autoVoice: this.config.autoVoice,
  ...
  /**
   * Delete cache files not referenced during this build (orphans left behind
   * after a post is edited or removed). Skips pruning when no posts were
   * processed, since that usually means an incremental/no-op build whose
   * reference set is incomplete — pruning then could wipe still-needed audio.
   *
   * @returns {number} Number of stale cache files removed.
   */
  pruneCache() {
    if (this.entries.size === 0) {
      this.log.info('hexo-reader: skipping cache prune (no posts processed this build)');
      return 0;
    }
    const removed = this.cache.prune(new Set(this.entries.keys()));
    if (removed.length > 0) {
      this.log.info(`hexo-reader: pruned ${removed.length} stale cache file(s)`);
    }
